...
def user_input():
    ...
    entry_rev_points.insert(0, "80")  # Set default value to 80
...
def tecplot_exporter_modal(file_path, rev_angle, divisions, mesh, eig_vect,deformation):
    ...
    with open(file_path, 'w') as file:
        file.write("TITLE = \"Shell\"\n")
        file.write("VARIABLES = x, y, z, w\n")

        def_mesh = np.copy(mesh)    # Left undeformed for reference
        ...
        for i in range(0, divisions):
            for j in range(0, n_nodes):
                file.write(f'{float(x[j,i]):.7e}  {float(y[j,i]):.7e}  {float(def_mesh[j,0]):.7e}  {float(0):.7e}\n')
        
        # Deformed Shell Modes in each ZONE
        for k in range(10):
            def_mesh = np.copy(mesh)
            def_mesh[:,0] += eig_vect[0::3,k]*(deformation*350)
            def_mesh[:,1] += eig_vect[1::3,k]*(deformation*350)
            ...
            for i in range(0, divisions):
                for j in range(0, n_nodes):
                    file.write(f'{float(x[j,i]):.7e}  {float(y[j,i]):.7e}  {float(def_mesh[j,0]):.7e}  {float(eig_vect[3*j+1,k]):.7e}\n')
...
def teplot_exporter_2d(file_path, t_col, u_global_native, strains, stress, stress_memb, t_VM, fsy, fsu):
    ...
    for i in range(time_steps):
        for j in range(vars):
            if j >= vars-2:                             # if it is the last and before last value
                data_max[i,j] = np.min(u_global[:,j,i]) # The safety factors should be minima
            else:
                data_max[i,j] = np.max(u_global[:,j,i]) # Everything else looks for maxima
    ...
    with open(file_path, 'w') as file:
        ...
        for i in range(0, vars):
            ...
            file.write(f"ZONE T=\"{vars_name[i]}\", I={time_steps:04d}, F=POINT\n")
...
tecplot_exporter_dynamic_zones('FEM Analysis - Data\Dynamic Analysis\output_export_dynamic_deformed_tecplot_3d.txt',rev_degrees, rev_points, mesh,t_col_red, u_global_matrix, strains_d, stress_d, stress_memb_d, t_VM_d, fsy_d, fsu_d, 3)
